...
const fetchWithRetry = async (
  ...
  return Promise.reject(new Error("Failed to fetch after max retries"));
};
...
const callGeminiAPI = async (
  ...
  try {
    ...
    if (candidate && candidate.content?.parts?.[0]?.text) {
      ...
      return {
        text: "Error: The AI model returned an unexpected or empty response.",
        sources: [],
      };
    }
  } catch (error) {
...
const fetchServerUI = async (
  ...
  if (env.os === "iOS" || (env.os === "macOS" && env.deviceType === "Mobile")) {
    ...
    env.os === "Windows" ||
    (env.os === "macOS" && env.deviceType === "Desktop") ||
    env.os === "Linux"
  ) {
  ...
  return {
    ...
    theme: themeConfig,
    title: title,
    uiHint: `${env.os} components requested in ${responseMode} mode`,
  };
};
...
const processLLMInteraction = async (
  ...
  if (botMode === BOT_NAMES.LOCAL) {
    ...
    if (localVDB.length > 0) {
      ...
      answerText +=
        "My local data is empty. I cannot generate a deep response.";
    }
    ...
    if (!isOnline) {
      return {
        text: `[Offline - ${botMode}] Cannot connect to remote LLM (Gemini API) while offline.`,
        newVDBEntry: null,
      };
    }
...
const PublicTimeline = ({ chatHistory, theme }: PublicTimelineProps) => {
  ...
  return (
    <div className={`p-4 rounded-xl max-h-64 overflow-y-auto ${cardStyle}`}>
      <h3
        className={`flex items-center text-sm font-semibold mb-3 border-b ${lineStyle} pb-1 ${textStyle}`}
      >
      ...
      {items.length === 0 ? (
        <p
          className={`text-xs italic ${isDark ? "text-gray-500" : "text-gray-500"}`}
        >
        ...
        items.map((pair, index) => (
          <div
            key={index}
            className={`mb-3 p-2 rounded-lg transition-all duration-300 cursor-pointer ${hoverStyle}`}
          >
            <p
              className={`text-xs font-medium ${isDark ? "text-gray-400" : "text-gray-600"} opacity-90`}
            >
              {new Date(pair.user.timestamp).toLocaleTimeString()} - User ID:{" "}
              {pair.user.userId?.substring(0, 8) || "anon"}...
            </p>
            <div className="flex items-start mt-1">
              <div
                className={`w-1.5 h-1.5 rounded-full ${accentColor} flex-shrink-0 mt-1 mr-2`}
              ></div>
              <p
                className={`text-sm font-light leading-snug ${isDark ? "text-gray-100" : "text-gray-900"}`}
              >
                {pair.user.prompt}
              </p>
            </div>
            <p
              className={`text-xs italic mt-1 ml-4 ${isDark ? "text-gray-500" : "text-gray-500"}`}
            >
              <MessageSquare className="inline w-3 h-3 mr-1" />
              Bot reply seen: "
              {pair.bot.response?.substring(0, 40) || "[No response]"}..."
            </p>
          </div>
        ))
      )}
...
const MainOutputArea = ({
  ...
  const userBubbleBg = isDark
    ? "bg-blue-700"
    : `${theme.accent.replace("text-", "bg-")} bg-opacity-90`;
  ...
  const analyzeButton = (
    ...
    >
      <Lightbulb className="w-4 h-4 mr-2" />✨ Contextual Analysis
    </button>
  );
  ...
  if (responseMode.includes("Inside")) {
    mainContent = (
      <div
        className={`p-8 text-center rounded-xl shadow-xl border ${isDark ? "bg-gray-800 border-gray-700" : "bg-white bg-opacity-50 backdrop-blur-md border-gray-200"} flex items-center justify-center flex-grow w-full`}
      >
    ...
    mainContent = (
      <div className="flex flex-col items-center flex-grow w-full h-full p-4 relative">
        <h1
          className={`text-4xl md:text-6xl font-light tracking-tight ${primaryTextClass} text-opacity-80 mb-8 flex-shrink-0`}
        >
          {title.split("(")[0].trim()}
        </h1>
        ...
        <div className="w-full max-w-2xl flex-grow overflow-y-auto space-y-4 p-2">
          {chatHistory.length === 0 && (
            <div
              className={`p-8 rounded-xl text-center flex flex-col items-center justify-center h-full min-h-[200px] ${isDark ? "bg-gray-800 text-gray-300" : "bg-white text-gray-700"}`}
            >
              <p className="mb-2">Start a conversation or try a command:</p>
              <p className="text-sm italic">
                e.g., Ask about a topic, or use `/plan` or `/agent` to change
                bot persona.
              </p>
              <p className="text-sm italic mt-1">
                Use `/local` for client-side processing{" "}
                {isLocalModelReady ? "✅" : "⏳"}.
              </p>
              {analyzeButton}
            </div>
          ...
          {chatHistory.slice(-5).map((chat, index) => (
            ...
            >
              <p
                className={`text-xs font-semibold ${chat.role === "user" ? userBubbleText : botRoleText}`}
              >
              ...
              {chat.embedding && Array.isArray(chat.embedding) && (
                <p className="text-[10px] opacity-70 mt-1">
                  VDB: [{chat.embedding.join(", ").substring(0, 30)}...]
                </p>
              )}
              {chat.sources && chat.sources.length > 0 && (
                <div className="mt-2 pt-2 border-t border-current border-opacity-10">
                  ...
                  {chat.sources.slice(0, 2).map((s, i) => (
                    <a
                      ...
                      className="text-[10px] underline block hover:opacity-70 truncate"
                    >
                      {s.title}
                    </a>
                  ))}
  ...
  return (
    ...
    <div
      className={`flex-grow flex flex-col items-center justify-center w-full transition-colors duration-500`}
    >
      {mainContent}
...
const CommandLineInput = ({
  ...
  const modeIcon =
    botMode === BOT_NAMES.ASSISTANT ? (
      <Smile size={20} />
    ) : botMode === BOT_NAMES.PLAN ? (
      <Compass size={20} />
    ) : botMode === BOT_NAMES.LOCAL ? (
      <Cpu size={20} />
    ) : (
      <Zap size={20} />
    );
  ...
  return (
    ...
    <div className="p-4 md:p-8 flex-shrink-0">
      <div
        className={`flex items-center px-4 py-2 shadow-lg transition-all duration-300 ${inputBaseStyle} max-w-2xl mx-auto`}
      >
        <span className={`text-gray-400 mr-2`}>{modeIcon}</span>
  ...
  useEffect(() => {
    if (environmentData) {
      const fetchConfig = async () => {
        ...
        const config = await fetchServerUI(
          environmentData,
          currentLayout,
          isLoggedIn,
        );
  ...
  const handleBotCommand = (command: string) => {
    ...
    if (subCommand === "assistant") newMode = BOT_NAMES.ASSISTANT;
      ...
      if (!isLocalModelReady) {
        setChatHistory((prev) => [
          ...
          {
            role: "bot",
            response: "Cannot switch. Local model is still loading...",
            mode: "System",
            timestamp: Date.now(),
          },
        ]);
      ...
      setChatHistory((prev) => [
        ...
        {
          role: "bot",
          response: "Switched layout to: Outside. Fetching new configuration.",
          mode: "System",
          timestamp: Date.now(),
        },
      ]);
      ...
      if (!isLoggedIn) {
        setChatHistory((prev) => [
          ...
          {
            role: "bot",
            response: "Inside mode is restricted. Please use //login first.",
            mode: "System",
            timestamp: Date.now(),
          },
        ]);
      ...
      setChatHistory((prev) => [
        ...prev,
        {
          role: "bot",
          response:
            "Switched layout to: Inside. Fetching secure configuration.",
          mode: "System",
          timestamp: Date.now(),
        },
      ]);
      ...
      setChatHistory((prev) => [
        ...
        {
          role: "bot",
          response: "Logged out. Switching to Outside mode. UI re-fetching.",
          mode: "System",
          timestamp: Date.now(),
        },
      ]);
    ...
    if (newMode) {
      ...
      setChatHistory((prev) => [
        ...
        {
          role: "bot",
          response: `Switched bot mode to: ${newMode}`,
          mode: "System",
          timestamp: Date.now(),
        },
      ]);
      ...
      setChatHistory((prev) => [
        ...
        {
          role: "bot",
          response: `Unknown bot command: /${subCommand}`,
          mode: "System",
          timestamp: Date.now(),
        },
      ]);
...
Analysis Request: Summary, Topic, Next Action. Format the output clearly.`;
    ...
    try {
      const geminiResponse = await callGeminiAPI(
        ...
        systemPrompt,
        false,
      ); // No grounding needed for context analysis
      ...
      setChatHistory((prev) => [
        ...
        {
          role: "bot",
          response: "Error during analysis process.",
          mode: "System",
          timestamp: Date.now(),
        },
      ]);
  ...
  const handleChat = useCallback(async () => {
    ...
    try {
      ...
      if (botMode === BOT_NAMES.LOCAL) {
        ...
        result = await processLLMInteraction(
          currentPrompt,
          botMode,
          isOnline,
          localVDB,
      ...
      setChatHistory((prev) => [
        ...
        {
          role: "bot",
          response: "Error during conversation process.",
          mode: "System",
          timestamp: Date.now(),
        },
      ]);
  ...
  const handleLoginSuccess = () => {
    ...
    setChatHistory((prev) => [
      ...prev,
      {
        role: "bot",
        response:
          "Login successful! Adaptive UI re-fetching for secure access.",
        mode: "System",
        timestamp: Date.now(),
      },
    ]);
    ...
    return (
      <div className={`flex items-center justify-center h-screen ${bgClass}`}>
        <div className="text-center">
          <svg
            className={`animate-spin h-8 w-8 ${spinnerColor} mx-auto`}
          ...
          <p className={`mt-4 ${textColor}`}>
            Detecting environment and fetching adaptive UI...
          </p>
        </div>
      </div>
    );
  ...
  const getLocalModelStatus = () => {
    ...
    if (isLocalModelReady)
      return { text: "Ready (Local Mode: /local)", color: "text-green-500" };
  ...
  return (
    ...
    <div
      className={`flex flex-col h-screen ${bgClass} font-sans transition-colors duration-1000 overflow-hidden`}
    >
      <header
        className={`sticky top-0 z-10 p-4 ${headerBaseStyle} flex-shrink-0`}
      >
        <div className="flex justify-between items-center max-w-7xl mx-auto">
          <button
            className={`text-sm font-medium ${headerAccentText} hover:opacity-70 transition`}
          >
          ...
          <div className="flex space-x-4 items-center">
            ...
            <div
              className={`text-xs flex items-center hidden md:flex ${modelStatus.color}`}
            >
            ...
            <span className={`text-gray-500 hidden sm:inline text-sm`}>
              {environmentData.os} / {environmentData.deviceType}
            </span>
            <span className={`text-gray-700`}>
              {isLoggedIn ? (
                <User className="inline w-4 h-4 text-green-500" />
              ) : (
                <LogIn className="inline w-4 h-4 text-red-500" />
              )}
            </span>
            <button
              ...
              onClick={() => {
                const nextLayout =
                  currentLayout === "Outside" ? "Inside" : "Outside";
                if (nextLayout === "Inside" && !isLoggedIn) {
                  setChatHistory((prev) => [
                    ...prev,
                    {
                      role: "bot",
                      response:
                        "Inside mode is restricted. Please use //login first.",
                      mode: "System",
                      timestamp: Date.now(),
                    },
                  ]);
                ...
                setChatHistory((prev) => [
                  ...
                  {
                    role: "bot",
                    response: `Switched layout via button to: ${nextLayout}. UI re-fetching.`,
                    mode: "System",
                    timestamp: Date.now(),
                  },
                ]);
      ...
      {isLoading && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-10 z-40">
          <div
            className={`flex items-center p-4 rounded-xl shadow-2xl ${isDark ? "bg-gray-700" : "bg-white bg-opacity-90 backdrop-blur-md"}`}
          >
            <svg
              className={`animate-spin h-6 w-6 ${isDark ? "text-white" : "text-gray-500"}`}
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
            >
            ...
            <p
              className={`ml-3 text-sm font-medium ${isDark ? "text-white" : "text-gray-700"}`}
            >
              {botMode === BOT_NAMES.LOCAL
